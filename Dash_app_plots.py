...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', options=[{'label':'All Sites', 'value':'ALL'},
                                                                            ...
                                                                            {'label':'VAFB SLC-4E', 'value':'VAFB SLC-4E'}], value='ALL', placeholder='Launch Site', searchable=True),
                                html.Br(),
...
def get_pie_chart(entered_value):
    ...
    if entered_value=="ALL":
        fig = px.pie(spacex_df, values='class', names='Launch Site', title='pie chart of successfull')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site']==entered_value]
        filtered_df['count'] = 1
        filtered_df_g = filtered_df.groupby('class', as_index=False).sum('count')
        fig = px.pie(filtered_df_g, values='count', names='class', title='pie chart of successfull')
        return fig
# TASK 4:
